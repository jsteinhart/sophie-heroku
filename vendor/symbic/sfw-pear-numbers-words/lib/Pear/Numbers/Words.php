<?php
/**
 * Modified Version of Pear Package Number_Words allowing autoloading
 */
/**
 * Numbers_Words
 *
...
class Pear_Numbers_Words
{
	...
	{
		$classname = 'Pear_Numbers_Words_' . $locale;
		if (!class_exists($classname))
		{
			if (str_rpos($locale, '.') !== false)
			{
				throw new Exception('Invalid locale value passed to in Pear_Number_Words->toWords');
			}
			require(__DIR__ . '/Words/lang.' . $locale . '.php');
		}
		...
		{
			throw new Exception('Unable to load class ' . $classname . ' in Pear_Number_Words->toWords');
		}
		...
		try {
			$value = $obj->toWords($num);
		}
		catch(Exception $e)
		{
			throw new Exception('Unable to execute toWords for classname ' . $classname . ' in Pear_Number_Words->toWords');
		}
	...
	function toCurrency($num, $locale = 'en_US', $int_curr = '')
	{
		...
		{
			throw new Exception('Unable to execute toCurrency for classname ' . $classname . ' in Pear_Number_Words->toCurrency');
		}
	}
